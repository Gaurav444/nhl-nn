...
con = sql.connect (host = "127.0.0.1", user = "root", passwd = "root", db = "toyama")
con.autocommit(True)
...
def check_game(game):
	query_sql = "SELECT * FROM raw WHERE team_a = '%s' AND team_b = '%s' AND date='%s' AND a_score=%d AND b_score=%d LIMIT 1" % (game['team_a'],game['team_b'],game['date'],int(game['a_score']),int(game['b_score']))
	rows_count = c.execute(query_sql)
	if rows_count > 0:
		return c.fetchone()
	else:
...
def get_all(table='raw',order_col='date',direction='ASC',season=0):
	if season == 0:
		query_sql = "SELECT * FROM %s ORDER BY %s %s" % (table,order_col,direction)
	else:
		query_sql = "SELECT * FROM %s WHERE season=%d ORDER BY %s %s" % (table,season,order_col,direction)
	#print(query_sql)
	c.execute(query_sql)	
	return c.fetchall()
